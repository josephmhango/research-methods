...
## When the ANOVA assumptions are unsuitable

The Kruskal-Wallis test provides a rank-based comparison for three or more independent groups. It is especially useful when the outcome is ordinal, severe skew or influential outliers make a mean-based analysis difficult to defend, or the available data do not support the distributional assumptions used for the ANOVA model.

The hypotheses are:

- $H_0$: the groups have the same distribution of ranks;
- $H_A$: at least one group tends to produce different ranks.

When group distributions have similar shapes, the result is often interpreted as evidence about differences in location or medians.

## ANOVA and Kruskal-Wallis on the same example
...
The ANOVA F-test compares variation between group means with variation within groups, while Kruskal-Wallis converts observations to ranks and compares the rank patterns across groups.

## Visualise before choosing
...
boxplot(
  ...
  xlab = "Species",
  ylab = "Petal length",
  main = "Petal length across three species"
)
stripchart(
  Petal.Length ~ Species,
  data = iris,
...
A significant Kruskal-Wallis test shows that a difference exists somewhere among the groups and does not identify the differing pairs. Pairwise Wilcoxon rank-sum tests with an adjustment for multiple comparisons provide one accessible follow-up approach.
...
## Reporting the rank-based analysis

Report the Kruskal-Wallis statistic, degrees of freedom, p-value, group sample sizes, medians, and interquartile ranges. When follow-up tests are used, report the adjustment method and the adjusted pairwise p-values.

An example structure is: "Petal length differed among species, Kruskal-Wallis $H(2) = ...$, $p < ...$. Holm-adjusted pairwise comparisons indicated that ... . Group medians and interquartile ranges were ... ."
